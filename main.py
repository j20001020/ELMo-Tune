...
def main():
    ...
    generate new options files using Gemini API and run db_bench with the new options file. This function will also
    store the output of db_bench in a file. The output file will contain the benchmark results, the options file
    used to generate the benchmark results and the reasoning behind the options file as provided by the Gemini API.
    ...
    if is_error:
        ...
        # If the initial options file succeeds, store the options file and benchmark results, pass it to the Gemini API to generate a new options file
        ...
        for i in range(1, ITERATIONS + 1):
            ...
            log_update(f"[MFN] Querying Gemini for next options file")

            print("-" * 50)
            print(f"[MFN] Starting iteration {i}")

            print("[MFN] Querying Gemini for next options file")
            temperature = 0.4
            retry_counter = 5
            generated = False
